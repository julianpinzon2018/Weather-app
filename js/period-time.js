...
export function periodTimeTemplate({ temp, date, icon, description }, id) {
  const selected = id.indexWeather === 0 ? " is-selected" : "";
  return `
        <li class="dayWeather-item${selected}" id="summary-${id.index}${id.indexWeather}">
                  <span class="dayWeather-time">${date}</span>
                  <img
                    class="dayWeather-icon" height='48' width='48'
